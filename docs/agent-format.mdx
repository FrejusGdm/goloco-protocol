...
<Note>
`@goloco/agent-format` ships in this repository (`packages/agent-format`) with the spec, JSON schemas, validator, and examples. You can also publish an agent by hosting your own A2A-compatible agent card and registering its URL with [`goloco agent publish --agent-card-url`](/cli/overview).
</Note>
